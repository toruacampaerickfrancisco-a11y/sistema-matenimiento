...
// Definir un modelo simple para 'insumos' (si no existe en modelos)
...
async function run() {
  try {
    ...
    const users = await User.findAll({ attributes: ['id','nombre_completo','email'] });
    if (!users || users.length === 0) {
      console.error('No users found in DB. Aborting.');
      return process.exit(1);
    }

    // Insert sample insumos if they don't already exist
    const sampleInsumos = [
    ...
    for (let i=0;i<createCount;i++) {
      ...
      const assigned = Math.random() < 0.3 ? randItem(users) : null; // 30% assigned
      ...
      console.log('Created ticket:', ticket.ticket_number, '-', title, 'reportedBy:', reporter.nombre_completo || reporter.email);
    }
